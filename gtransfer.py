...
def transferDocument(client, stdToken, spreadsheetToken, feed, storeFolder, storeFlat, ignoreDualCollections):
	if not feed.entry:
		LOG.info ("No entries in feed - Nothing to backup")
	cleanStoreFolder(storeFolder)
	forceFolder(storeFolder)
	stats = {'copy':0, 'rejected':0, 'error':0}
	for entry in feed.entry:
		...
		# copy only for doc, spreadsheet and presentation type. See :
		...
		if entry.GetDocumentType()== "document" :
			...
			toCopy=True
		if toCopy: 
			duplicated_entry = client.Copy(entry, 'TRANSFER_'+entry.title.text.encode(sys.getfilesystemencoding()))
			LOG.info("Copy done : "+str(duplicated_entry.title.text.encode(sys.getfilesystemencoding())))
			stats['copy']+=1
			LOG.info("adding ACL (except mine)")
			client.auth_token = stdToken
			aclFeed = client.GetAclPermissions(entry.resource_id.text)
			for acl in aclFeed.entry:
				if acl.scope.value== login:
					continue
				if acl.role.value== "owner":
					continue
				LOG.info (' adding acl to copy doc: '+str(acl.scope.value)+' ('+str(acl.scope.type)+') is '+str(acl.role.value))
			...
			LOG.info("renaming orig")
			orig=entry.title.text
			entry.title.text="TOREMOVE_"+entry.title.text
			client.Update(entry)
			LOG.info("renaming copy "+orig)
			duplicated_entry.title.text=orig
			client.Update(duplicated_entry, force=True)
		else:
			LOG.info("Cant' copy that kind of document : "+entry.GetDocumentType())
			stats['rejected']+=1
...
if __name__ == '__main__':
	...
	LOG.info ("    -> succès")
	LOG.info ("Récupération de la liste de tous les documents de %s"%args.login)
	...
	login = args.login
	stats = transferDocument(client, client.auth_token, gdata.gauth.ClientLoginToken(spreadsheets_client.GetClientLoginToken()), feed, folder, args.flat, args.ignore)
